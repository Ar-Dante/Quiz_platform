ERROR_CONNECTING_TO_DB = "Error connecting to the database"
ERROR_DB_NOT_CONFIGURED = "Database is not configured correctly"

ERROR_USER_NOT_FOUND = "User not found"
ERROR_ACCOUNT_EXISTS = "User already exists"
ERROR_INVALID_CRED = "Invalid cred"

ERROR_INVALID_TOKEN = 'Invalid scope for token'
ERROR_CANT_VALIDATE_CRED = 'Could not validate credentials'
ERROR_ACCESS = "Access denied"

ERROR_COMPANY_NOT_FOUND = "Company not found"
ERROR_QUIZ_NOT_FOUND = "Quiz not found"
ERROR_QUESTION_NOT_FOUND = "Question not found"
ERROR_USER_NOT_INVITED = "The user has not been invited "
ERROR_USER_INVITED = "User already invited"
ERROR_USER_REQUEST = "Request was already sent"
ERROR_USER_NOT_REQUESTED = "The user did not send a request"

ERROR_MEMBER_NOT_FOUND = "Member not found"
ERROR_MEMBER_EXISTS = "User is already in the company"
ERROR_MEMBER_NOT_EXISTS = "User is not in the company"
ERROR_MEMBER_ADMIN = "User is already an admin in the company"
ERROR_MEMBER_NOT_ADMIN = "User is not an admin in the company"
ERROR_MEMBER_OWNER_ADMIN = "The owner can't be an admin"
ERROR_MEMBER_OWNER = "The owner can't be member"

ERROR_NOT_ENOUGH_OPTIONS = "A question must have at least two answer options."
ERROR_NOT_ENOUGH_QUESTIONS = "A quiz must have at least two questions options."
